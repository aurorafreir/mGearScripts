...
TYPE = "digi_leg_02"
NAME = "control"
DESCRIPTION = "Digitigrade leg component :)."
...
class Guide(guide.ComponentGuide):
    ...
    # @param self
    def postInit(self):
        self.save_transform = ["root", "toe", "paw", "ankle", "knee", "hip"]
    ...
    def addObjects(self):
        ...
        self.hip = self.addLoc("hip", self.root, [0, 15, 0])
        self.knee = self.addLoc("knee", self.hip, [0, 10, 2])
        self.ankle = self.addLoc("ankle", self.knee, [0, 5, -2])
        self.paw = self.addLoc("paw", self.ankle, [0, 0, 0])
        self.toe = self.addLoc("toe", self.paw, [0, 0, 2])

        centres = [self.hip, self.knee, self.ankle, self.paw, self.toe]
        # Create display lines between each control
        self.dispcrv = self.addDispCurve("crv", centres)
